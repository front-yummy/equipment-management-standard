...
| 角色 | 描述 | 核心职责 |
|------|------|----------|
| 设备管理员 | 设备基础数据的维护者 | 设备台账维护、分类配置、BOM管理、停机分类维护 |
...
**概述**：建立设备分类体系（设备组、设备类型）和统一的故障原因分类标准（停机分类树），为设备管理提供标准化基础框架。
...
3. **停机分类配置**
   - 作用：为一类或一台设备预定义多级故障原因选项（如：一级-电气故障 → 二级-电机故障 → 三级-过载），用于后续维修工单的标准化记录
   - 配置层级：
     - **设备类型级**：在设备类型管理页面配置，作为该类型所有设备的默认分类标准
     - **设备级**：在单个设备台账编辑页面配置，用于覆盖或细化默认标准，实现个性化管理
   - 一键同步：修改设备类型的停机分类后，可一键应用到该类型下所有现有设备，实现标准统一
...
设备故障维修模块涵盖从叫修、派单、接单、签到、维修执行到完工的完整闭环。该模块的核心数据源有两个：

| 数据源 | 说明 |
|--------|------|
| **OEE 数据填报** | 生产人员在 OEE 数据填报中录入设备停机记录时，勾选"需要其他部门协助"，系统自动生成异常工单 |
| **手动叫修** | 用户在异常处理页面手动创建叫修申请，填写设备、故障描述等信息 |

### 5.2 异常工单来源与触发机制

#### 5.2.1 由 OEE 数据填报触发生成

当生产人员在 OEE 数据填报中录入一段停机记录时：

1. 填写停机时间段（开始时间、结束时间）
2. 选择停机分类（一/二/三级损失分类）
3. 填写停机详情、报警详情
4. 选择"是否需要其他部门协助"
   - **是** → 系统自动生成异常工单，并推送到指定责任部门/责任人
   - **否** → 仅记录停机数据，不生成工单
5. 提交后，通过系统通知或已配置的企业消息渠道推送给责任人
...
|------|------|------|
| 工单编号 | 系统自动生成 | 系统 |
| 设备编码 | 关联设备台账 | OEE填报自动带出/手动选择 |
| 设备名称 | 自动反显 | 设备台账 |
| 设备类型 | 自动反显 | 设备台账 |
| 线体 | 自动反显 | 设备台账 |
| 工序 | 自动反显 | 设备台账 |
| 责任部门 | 由停机分类配置或手动指定 | 配置/手动 |
| 责任人 | 推送到具体人员 | 手动 |
| 保修人 | 创建工单的人员 | 系统 |
...
生产现场人员基于实际生产情况，按设备/班次维度填报停机、损失数据。这是 OEE 计算和异常工单触发的原始数据入口。
...
在设备类型管理页面，以树形结构维护损失分类。支持导入/导出、一键同步到设备类型下所有设备。
...
**概述**：管理系统内使用的数据字典（如停机分类、备件分类、设备状态等），支持动态配置字典项。
...
| [01-设备资产管理.md](需求分析/01-设备资产管理.md) | 设备台账、设备类型、停机分类、BOM、履历 | 台账页面、详情页、主数据接口 |
| [02-预防性维护.md](需求分析/02-预防性维护.md) | 点检、巡检、保养的基准、计划、任务、验收 | 计划任务引擎、移动端执行页 |
| [03-故障维修与异常工单.md](需求分析/03-故障维修与异常工单.md) | OEE 触发工单、叫修、派单、接单、签到、完工、协助 | 工单状态机、通知渠道、维修执行页 |
| [04-OEE与KPI分析.md](需求分析/04-OEE与KPI分析.md) | OEE 填报、损失记录、PPM、KPI 看板、下钻 | 报表计算、设备状态采集对接、指标看板 |
...
| [07-预警通知与逐级上报.md](需求分析/07-预警通知与逐级上报.md) | 任务逾期、备件寿命、库存水位、通知渠道 | 通知中心、升级规则、提醒任务 |
