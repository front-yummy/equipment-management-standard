...
- [9. 模块7：通用预警逐级上报机制](#9-模块7通用预警逐级上报机制)
  - [9.1 功能描述](#91-功能描述)
  - [9.2 配置与逻辑](#92-配置与逻辑)
- [10. 报表统计](#10-报表统计)
  - [10.1 功能概述](#101-功能概述)
...
首页仪表盘是系统登录后的默认页面，面向设备管理员、生产管理者等角色，以卡片和图表形式集中展示关键 KPI 指标、待办任务、设备状态分布及快捷操作入口，帮助用户快速掌握设备运营状况。
...
| **设备状态分布** | 饼图/环形图 | 按运行/停机/维修/闲置等状态统计设备数量及占比 |
...
**概述**：建立设备分类体系（设备组、设备类型），为设备建档、预防性维护基准和业务筛选提供基础框架。损失/停机分类不放在设备类型中维护，由 OEE 模块配置实现。
...
2. **设备类维护**
   - 设备类型的增删改查
   - 字段：设备类型编码（必填）、设备类型名称（必填）

3. **损失/停机分类归属**
   - 损失/停机分类用于 OEE 损失填报、责任归因、维修统计和 KPI 下钻。
   - 标准产品不在 base 或设备类型中维护停机分类，统一在 OEE 模块内配置。
   - 维修工单可引用 OEE 分类结果用于归因和统计，但分类维护入口不放在维修工单或设备台账。

### 3.2 设备台账信息全生命周期管理

**概述**：统一维护设备主数据，实现设备基础信息（编码、名称、型号、位置、技术参数等）集中管理，支持多种维护方式，为每台设备生成唯一二维码标识。

**功能点**：

1. **新增设备**
   - 表单字段分组：
     - **简要信息**：设备编号、设备名称、设备简称、资产状态、数采编号（与数采关联）
     - **基本信息**：资产编号、设备单位、设备类型编码&设备类型、重要级别、设备用途、设备型号、尺寸、重量、主要技术性能参数、设备功率、所属组织、使用组织、所属部门、设备位置、所属工段、负责人、设备组、投产时间、使用寿命、是否生产设备、是否计量设备、是否特种设备、备注
     - **采购信息**：制造商、供应商、供应商电话、生产日期、出厂编号、产地、购置时间
     - **设备图片**：可上传图片
     - **设备附件**：文件上传列表，包含文件名称、文件说明、文件类型、文件大小、上传时间、上传人
   - 规则：
      - 设备编号全局唯一；设备编号、名称、所属类型、所属线体必填
      - 当是否生产设备、是否计量设备、是否特种设备为是时，会有相应的属性名称和属性值（支持多条）。

2. **导入设备**
   - 下载系统提供的 Excel 模板 → 批量填写设备数据 → 上传
   - 系统异步处理，含文件格式校验、数据合法性校验（编码重复、引用值不存在）
   - 导入失败时可下载错误报告

3. **编辑/删除设备**
   - 编辑：修改已存在设备的信息
   - 删除：单条与批量删除均需二次确认；检查是否存在关联业务数据（点检计划、维修工单等），有关联则阻止删除

4. **筛选与查询**
   - 列表页支持设备编号、名称、类型、重要级别等多条件组合快速查找

5. **导出**
   - 当前列表（全部或选中项）导出为 Excel 文件

6. **批量生成二维码**
   - 选择多台设备，系统生成包含设备基本信息的二维码图片包供下载
   - 现场人员用移动端扫码即可快速调取设备详细信息，扫码终端可按项目选择手机、PDA 或扫码枪

### 3.3 设备全景履历详情

**概述**：在一个页面内聚合展示设备所有静态信息和动态历史记录，采用多标签页（Tab）形式呈现，用户无需在多个模块间切换即可全面掌握设备状况。

**功能点**：

1. **多标签页设计**：
   - **基本信息 Tab**：展示设备所有基础信息（同新增页面字段）
   - **设备 BOM Tab**：设备部件清单，树形结构展示，含部件类型（部件/备件）、部件编码、部件型号、名称、制造商、数量、单位、理论寿命（时间-天/产量-EA）、部件图片,当部件类型是备件时，关联备件模块，通过部件编码带出名称、型号、制造商、单位信息。
   - **验收履历 Tab**：展示设备所有验收记录，含验收单名称、编号、验收完成时间、验收人、验收单类型、附件、验收备注，支持增删改查。
   - **保养履历 Tab**：展示设备所有保养记录，同步保养模块数据，含保养任务编号、责任人、保养计划编号、责任部门、保养机制、计划保养时间、任务状态、开始执行时间、结束执行时间、应检项目数、漏检项目数、异常项目数。 
   - **点巡检履历 Tab**：
      - 计划/任务记录列表：按时间范围筛选。
      - 任务统计看板：任务总数、已完成数、待处理数、已逾期数看板（按计划执行时间筛选）。
      - 检查项看板：
         - 总检查项数。
         - 正常项占比、异常项占比、漏检项占比。
         - 未达成项数。
         - 任务完成率饼图（正常项数 ÷ 总检查项 × 100%）。
         - 柱状图展示正常项、异常项、漏检项对比。
   - **维修履历 Tab**：暂无。
   - **备件履历 Tab**：该设备所有备件更换记录，含备件编码、备件名称、领用单号、领用日期、备件序列号、备件绑定状态（已绑定/已解绑）、绑定日期、解绑日期。
   - **操作日志 Tab**：展示设备所有操作记录，含操作人、操作时间、操作内容（如新增/修改设备，部分字段支持前后对比展示）。
...
**DT 率趋势图**：筛选工厂、拉线、工段、工序、设备；目标 DT 率 vs 实际 DT 率对比；时间跨度：近 3 周 / 近 3 个月 / 近 1 季度 / 近 2 年
...
2. **知识条目创建与管理**
   ...
   - **故障排除列表**：以表格形式逐条录入，包含设备状态、故障代码、原因、类型、描述、举措、关联部件、平均修复时间（MTTR）、预防性建议等结构化字段
...
当任务逾期时，按预设的逐级上报规则自动升级通知，确保问题不被遗漏。

### 9.2 配置与逻辑

**开关**：全局开关控制逐级上报是否启用（如：点检任务逾期逐级上报）

**上报级别**：班组长 → +1 级 → +2 级 → +3 级

每个级别配置：
- 延期时间（分钟）：超过此时间未处理则升级到下一级
- 通知渠道：系统通知/企业 IM/短信/邮件等可配置渠道

**触发逻辑**：
- 任务逾期 → 触发一级上报（通知班组长）
- 上级在配置的延期时间内未处理 → 自动升级到下一级
- 任务完成后，停止所有上报与提醒
...
**概述**：系统管理不维护业务主数据和业务分类。业务主数据归 base 模块，OEE 损失/停机分类归 OEE 模块，系统管理只维护登录、权限、菜单和系统通用配置。
...
| [01-base主数据.md](需求分析/01-base主数据.md) | 工厂建模、生产关系、物料、工艺、BOM、合作伙伴、员工、主数据接入 | 主数据接口、数据权限引用、业务下拉 |
| [02-warehouse仓库.md](需求分析/02-warehouse仓库.md) | 仓库、库位、库存余额、入库、出库、盘点、库存流水 | WMS 对接、库存闭环、出入库回写 |
| [03-设备/01-设备台账.md](需求分析/03-设备/01-设备台账.md) | 设备建档、BOM、二维码、履历 | 台账页面、详情页、设备基础接口 |
| [03-设备/02-预防性维护.md](需求分析/03-设备/02-预防性维护.md) | 点检、巡检、保养的基准、计划、任务、验收 | 计划任务引擎、移动端执行页 |
| [03-设备/03-维修工单.md](需求分析/03-设备/03-维修工单.md) | OEE 触发工单、叫修、派单、接单、签到、完工、协助 | 工单状态机、通知渠道、维修执行页 |
| [03-设备/04-OEE.md](需求分析/03-设备/04-OEE.md) | OEE 填报、损失记录、PPM、KPI 看板、下钻 | 报表计算、设备状态采集对接、指标看板 |
| [03-设备/05-备件管理.md](需求分析/03-设备/05-备件管理.md) | 备件台账、领用、采购建议、PR/PO 跟踪、绑定、寿命 | 备件业务闭环、采购集成、寿命预警 |
| [04-AI知识库与智能助手.md](需求分析/04-AI知识库与智能助手.md) | 独立 AI 知识库，服务设备模块的维修推荐、ChatAI、指标分析 | AI 接口、问答入口、推荐反馈 |
| [05-消息通知与预警配置.md](需求分析/05-消息通知与预警配置.md) | 任务逾期、备件寿命、库存水位、流程待办、通知渠道 | 通知中心、升级规则、提醒任务 |
| [06-工作流配置.md](需求分析/06-工作流配置.md) | 审批、确认、退回、作废、流程节点、业务单据流程配置 | 工作流配置、流程实例、业务状态回写 |
| [07-系统管理.md](需求分析/07-系统管理.md) | 登录、用户、角色、菜单、权限、数据权限、运营报表 | 后台配置、权限矩阵、运营报表 |
