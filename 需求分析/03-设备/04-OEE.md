...
| 分类名称 | 文本 | 是 | 展示名称 |
| 上级分类 | 树选择 | 否 | 支持多级分类 |
| 分类类型 | 下拉 | 是 | 停机/待料/堵料/其他损失 |
| 默认责任部门/责任人 | 选择 | 否 | 可作为 OEE 填报默认值，允许人工调整 |
| 是否启用 | 开关 | 是 | 停用后历史保留，新损失不可选 |
| 导入/导出 | 操作 | 否 | 支持模板下载、导入、导出和错误报告 |
...
7. 损失/停机分类支持模板下载、导入、导出；导入失败可下载错误报告。
8. 选择协助且未手动指定责任班组时，可带出设备默认责任班组；设备责任人只作为责任候选和通知对象。
