...
| OEE 损失详情/时间明细 | 编辑停机分类、责任班组、责任部门、协助需求、原因措施 |
| 批量填报 | 批量维护损失分类和责任信息 |
| 损失/停机分类配置 | 维护 OEE 损失分类、停机类型、责任规则、启用状态 |
...
2. 班次根据开始时间自动判断。
3. 损失/停机分类由 OEE 模块配置，用于统一损失原因、责任归因和报表下钻口径，不由 base 或设备台账承接。
4. 是否需要其他部门协助默认为否；选择是时，按责任班组/责任部门过滤责任人，默认责任班组可取设备台账默认责任班组。
5. 异常责任人默认当前登录人或设备责任人，设备责任人只作为责任候选和通知对象，最终以用户选择为准。
...
| 状态与分类 | 一级损失分类 | 级联选择 | 是 | OEE 分类配置 |
| 状态与分类 | 二级损失分类 | 级联选择 | 否 | 依赖一级 |
| 状态与分类 | 三级损失分类 | 级联选择 | 否 | 依赖二级 |
| 责任信息 | 责任部门 | 部门选择 | 条件必填 | 需要协助或分类要求时必填 |
| 责任信息 | 责任班组 | 班组选择 | 否 | 可默认带出设备默认责任班组，用于协助工单派单候选 |
| 责任信息 | 责任人 | 用户选择 | 条件必填 | 按责任班组或责任部门过滤 |
